...
```{r, message=FALSE}
data <- read_delim("../../info201/PS5/gapminder.csv")
nrow(data)
ncol(data)
```    
...
```{r, message=FALSE}
data %>% 
  filter(name == "China" | name == "Angola" | name == "India" | name == "U.S" | name == "Australia") %>% 
  ggplot(aes(co2,
                   time,
                   col=factor(name))) + geom_line() +
  geom_point()
```
All countries shows a rapid increase in CO2 emission.
...
data %>% 
  ...
  geom_point()
```
It shows the general tendency of CO2_PC, but the graph is different than CO2 emmison vs time.
